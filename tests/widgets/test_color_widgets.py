...
from psyplot_gui.compat.qtcompat import QtGui
...
class CMapWidgetTest(bt.PsyPlotGuiTestCase):
    ...
    def setUp(self):
        ...
        self.project = psy.plot.plot2d(
            self.get_file(osp.join("..", "test-t2m-u-v.nc")), name="t2m"
        )
...
class BoundsWidgetTest(bt.PsyPlotGuiTestCase):
    ...
    def test_array(self):
        bounds = np.arange(280.0, 290.1, 1)
...
class CTicksWidgetTest(bt.PsyPlotGuiTestCase):
    ...
    def test_instance(self):
        """Test changes"""
        self.assertIsInstance(self.fmt_widget.fmt_widget, pswc.CTicksFmtWidget)
